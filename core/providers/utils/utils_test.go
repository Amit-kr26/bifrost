...
func TestDrainNonSSEStreamReader_TrimmableOnlyFirstReadIsStillSSE(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			// chunkSize 3 makes the first Read deliver exactly the trimmable
			// bytes and nothing else, which is what an upstream flushing the BOM
			// separately from the first field name looks like on the wire.
			reader, nonSSE := DrainNonSSEStreamReader(resp, &shortReadReader{data: tc.body, chunkSize: 3})
			if nonSSE != nil {
