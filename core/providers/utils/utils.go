...
func hasSSEPrefix(reader *bufio.Reader) (bool, error) {
	...
	window = trimSSELeading(window)
	if len(window) == 0 {
		// The first read carried nothing but a BOM or horizontal whitespace, so
		// the buffered window says nothing about the stream. Block for one more
		// byte rather than write a valid SSE body off as non-SSE; the idle
		// timeout reader bounds that wait, so a stalled upstream still surfaces
		// as StreamBodyIdleTimeout instead of a wrong verdict.
		more, err := reader.Peek(n + 1)
		if err != nil {
			return false, err
		}
		if window = trimSSELeading(more); len(window) == 0 {
			return false, nil
		}
	}
	switch window[0] {
