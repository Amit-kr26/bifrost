...
func coordinatedWarmSemanticExemplars(
	...
	// error is treated as "nobody else is doing this".
	claimed, err := coordinator.Claim(key, semanticWarmClaimTTL)
	if err != nil {
		claimed = true
	}
	...
	if claimed {
		...
		if warmErr != nil {
			...
			// generation that is not coming.
			coordinator.Release(key)
			return loaded, namespace, dimension, warmErr
		}
		if err := coordinator.Publish(key, WarmGeneration{Dimension: dimension, Namespace: namespace}, semanticWarmPublishTTL); err != nil {
